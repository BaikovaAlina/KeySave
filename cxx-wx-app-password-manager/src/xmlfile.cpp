...
void CXMLFile::read(const wxString& filePath, unsigned char* data, unsigned int& size)
{
    wxFile file;
    bool bResult = false;

    bResult = file.Open(filePath, wxFile::read);
    if (bResult == false)
        throw "File not open";
...
void CXMLFile::write(const wxString& filePath, const unsigned char* data, unsigned int size)
{
    wxFile file;
    bool bResult = false;

    bResult = file.Open(filePath, wxFile::write);
    if (bResult == false)
        throw "File not open";
...
void CXMLFile::parse(const unsigned char* inData, unsigned int inSize, TVectorItems& outData)
{
...
void CXMLFile::collect(const TVectorItems& inData, unsigned char* outData, unsigned int& outSize)
{
    wxXmlDocument xmlDoc;
