...
#define JDK "/usr/lib/jvm/java-21-openjdk-amd64"
...
CJNICloudLib::~CJNICloudLib()
{
    ...
    if(mInstance != nullptr)
        dlclose(mInstance); // Закрываем библиотеку
#endif // DYNAMIC_JDK_LINK
}
...
#ifdef DYNAMIC_JDK_LINK
    mInstance = dlopen(JDK "/lib/server/libjvm.so", RTLD_LAZY);
    if(mInstance == nullptr)
        throw "error: library Java VM not load";

    JNI_CreateJavaVM = (pCreateJavaVM)dlsym(mInstance, "JNI_CreateJavaVM");
