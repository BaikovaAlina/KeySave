...
our %config = (
  ...
  openssldir => "/etc/ssl",
  options => "--prefix=/usr --openssldir=/etc/ssl enable-shared enable-zlib-dynamic no-asan no-buildtest-c++ no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-ssl-trace no-ssl3 no-ssl3-method no-ubsan no-unit-test no-weak-ssl-ciphers",
  perl_archname => "x86_64-linux-thread-multi",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.40.0",
  perlargv => [ "linux-x86_64", "--prefix=/usr", "--openssldir=/etc/ssl", "shared", "zlib-dynamic" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
